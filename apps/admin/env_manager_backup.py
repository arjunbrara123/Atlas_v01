...
It implements the "Purpose-Driven Smart UI" (Option 3):
- The "Create / Clone" tab is a smart wizard.
- It dynamically changes the UI based on the selected 'env_cat'.
- 'Production' users get a simple, safe UI.
- 'Development' users get a "max flexibility" UI.
- 'Testing' users get a "full, realistic" UI.

This file is a "dumb" UI. It makes calls to the 'registry_service.py'
to perform all database and file system operations.
"""
...
class Page:
    ...
    def _render_dashboard_tab(self):
        ...
        st.caption("A view of the last 100 *human actions* taken on environments in the registry (from Table 7).")
        try:
    ...
    def _render_create_wizard_tab(self):
        ...
        create_method = st.radio(
            ...
            ["Create new empty environment (Start from scratch)",
             "Clone an existing environment (Start from a copy)"],
            key="create_method",
        ...
        with st.form("create_env_form"):
            ...
            if "Clone" in create_method:
                st.markdown("##### 3. Clone Settings (Dynamic)")
                if not self.all_envs:
                ...
                source_env_id = st.selectbox(
                    "Select Source Environment to Clone",
                ...
                if env_cat in ["Development", "Testing"]:
                    ...
                    if clone_data_only:
                        ...
                        folders_to_clone = [] # Will create empty folders

                elif env_cat in ["Reporting", "Validation"]:
                    # "Governor" Mode (Read-Only)
            ...
            if submitted:
                if not all([env_id_suffix, env_cat, purpose, allowed_roles, comment]):
                    ...
                    st.error("You must select a source environment to clone.")
                else:
                    action_noun = "Cloning" if source_env_id else "Creating"
                    with st.spinner(f"{action_noun} '{final_env_id}'... This may take a few minutes if cloning."):
    ...
    def _render_promote_validate_tab(self):
        """UI for high-governance actions: Promoting and Validation cloning."""
    ...
    def _render_manage_tab(self):
        ...
        st.markdown("Use this tab to **Edit** an environment's metadata (e.g., name or roles), **Archive** an old environment, or view its **Audit History**.")
